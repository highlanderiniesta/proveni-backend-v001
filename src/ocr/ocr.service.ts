...
// Importação do pdf2json (para extrair texto nativo)
...
export class OcrService {
  ...
  async extractTextFromPDF(filePath: string): Promise<string> {
    ...
    // 1. Tenta extrair texto nativo (PDF com camada de texto)
    ...
    // 2. Se não tem texto nativo, aplica OCR nas imagens das páginas
  ...
  private async extractNativeText(filePath: string): Promise<string> {
    return new Promise((resolve) => {
      ...
      pdfParser.on('pdfParser_dataError', (errData: any) => {
        const errorMsg = errData?.parserError || errData?.message || 'Erro desconhecido';
        this.logger.debug(`pdf2json erro (normal para PDF escaneado): ${errorMsg}`);
        resolve('');
      });
  ...
  /**
   * Converte PDF escaneado em imagens (via pdf2pic) e aplica OCR
   */
  async extractTextFromScannedPDF(filePath: string): Promise<string> {
    this.logger.log(`Convertendo PDF para imagens: ${path.basename(filePath)}`);
    ...
    try {
      ...
      // O resultado pode ser um array de objetos ou um único objeto
      ...
      for (const image of images) {
        // pdf2pic retorna objeto com { page, path }
        const imgPath = image.path;
        if (!imgPath || !fs.existsSync(imgPath)) continue;

        this.logger.log(`📸 Aplicando OCR na página ${image.page}: ${path.basename(imgPath)}`);
        ...
        await fs.promises.unlink(imgPath).catch((err) => {
          this.logger.warn(`Não foi possível remover ${imgPath}: ${err.message}`);
        });
      }

      this.logger.log(`✅ OCR concluído: ${fullText.length} caracteres`);
  ...
  async extractTextFromImage(filePath: string): Promise<string> {
    ...
    try {
      ...
      // Pós-processamento: remove espaços extras, caracteres estranhos
      text = text
        .replace(/\s+/g, ' ')          // normaliza espaços
        .replace(/(\d)\s+(\d)/g, '$1$2')      // remove espaços entre dígitos
        .replace(/(\d)\s+\.\s+(\d)/g, '$1.$2') // corrige números decimais
        .replace(/[^\x20-\x7E\u00C0-\u00FF\n]/g, '') // mantém apenas texto legível
        .trim();

      this.logger.log(`✅ OCR concluído: ${text.length} caracteres`);
      if (text.length > 0) {
        this.logger.debug(`📄 Amostra: "${text.substring(0, 200)}"`);
      }
      return text;
    } catch (error: any) {
  ...
  /**
   * Extrai texto de arquivo TXT/CSV/XML/JSON
   */
  ...
  /**
   * Extrai texto de qualquer tipo de arquivo (detecta pela extensão)
   */
  ...
  /**
   * Método para testar se o PDF tem texto (útil para diagnóstico)
   */
